...
You can add a filter or parameter widget to:
...
Heading cards. Not text cards, only heading cards. Can only be connected to cards on the current tab.
...
Connect your filter or parameter widget to one or more dashboard cards. Only dashboard-level widgets can be connected to cards on multiple tabs.
...
If you want a date picker where people can select multiple dates, you’ll need to change the variable in your SQL code to a field filter.
...
Year
To connect a time grouping widget to a SQL question, you’ll need to insert time grouping parameters in your code. See time grouping parameters.
...
Is. Select one or more values from a list or search box. Use Is when you just want to plug a value into a variable. For multiple values, the card must be either a question built using the query builder, a native/SQL question using a field filter, or a basic variable set to accept multiple values.
...
Is. Select one or more values from a list or search box. Use Is when you just want to plug a value into a variable. For multiple values, the card must be a question built using the query builder, a native/SQL question using a field filter, or a basic variable set to multiple values.
Is not. Exclude one or more specific values.
Contains. Match values that contain the entered text.
Does not contain. Filter out values that contain the entered text.
Starts with. Match values that begin with the entered text.
...
Where the widget is located: on the dashboard, heading card, or question card. Only dashboard widgets can be connected to cards across all dashboard tabs. Widgets on heading and question cards are restricted to the cards on their respective tabs.
...
To connect a dashboard filter widget to a card with a question created with the native editor, you’ll need to add a field filter or basic variable to your query.

For parameters, check out Time grouping parameters.
...
Rename filter
In dashboard edit mode, click the filter you want to edit.
...
Input box
Dropdown list
A list of all the possible values in a column. People can use checkboxes to select more than one value on multi-select filters. If you want the list of filter values to load instantly (from cache), you should choose the dropdown widget.

If you’re not seeing the Dropdown list option and your dashboard filter is based on a column from a:

Table or model built with the query builder: an admin will need to enable the dropdown widget for that column from Metabase’s Admin settings.
Model built with a SQL query: go to your model’s metadata settings, find your column, and set the Database column this maps to.
Search box
...
View the dashboard (not in edit mode) and click the three-dot menu ….
Select Edit settings.
In the General section, toggle off Auto-apply filters.
